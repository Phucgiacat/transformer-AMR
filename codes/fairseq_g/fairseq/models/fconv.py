...
class FConvModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        return FConvModel(encoder, decoder, g_encoder)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask, graph_encoder_out=None, graph_encoder_mask=None):
        residual = x
...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        adaptive_softmax_cutoff=None, adaptive_softmax_dropout=0, with_graph=False,
    ):
        super().__init__(dictionary)
        ...
        if self.with_graph:
            self.g_attention = GraphAttentionLayer(embed_dim, embed_dim, embed_dim)
            # self.highway = graph_encoder.Highway(embed_dim, embed_dim)
    ...
    def forward(self, prev_output_tokens, encoder_out=None,  graph_encoder_out=None, incremental_state=None, **unused):
        ...
        if graph_encoder_out is not None:
            g_encoder_out, g_encoder_mask  = graph_encoder_out.values()
        if incremental_state is not None:
            prev_output_tokens = prev_output_tokens[:, -1:]

        if self.with_graph:
            g_hidden = g_encoder_out[0]

        x = self._embed_tokens(prev_output_tokens, incremental_state)
        batch_size, t_len, embedding_dim = x.shape
        # embed tokens and combine with positional embeddings
        x += pos_embed
        if self.with_graph:
            x += g_hidden.unsqueeze(1).repeat(1, t_len, 1)
        x = F.dropout(x, p=self.dropout, training=self.training)
        target_embedding = x
        ...
        if self.with_graph:
            g_hidden, _ = self.g_attention(g_hidden, g_encoder_out[1].permute(1, 0, 2), g_encoder_mask)
        ...
        for proj, conv, attention, res_layer in zip(self.projections, self.convolutions, self.attention,
            ...
            if attention is not None:
                ...
                if self.with_graph:
                    x = x + g_hidden.unsqueeze(1).repeat(1, t_len, 1)
