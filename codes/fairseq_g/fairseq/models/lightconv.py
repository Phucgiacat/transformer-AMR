...
class LightConvModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, encoder, decoder, graph_encoder):
        super().__init__(encoder, decoder, graph_encoder=graph_encoder)
    ...
    def build_model(cls, args, task):
        ...
        return LightConvModel(encoder, decoder, g_encoder)
    ...
    def forward(self, prev_output_tokens, encoder_out=None, graph_encoder_out=None, incremental_state=None, **kwargs):
        ...
        for layer in self.layers:
            x, attn = layer(
                ...
                incremental_state,
                graph_encoder_out['encoder_out'] if graph_encoder_out is not None else None,
                graph_encoder_out['encoder_padding_mask'].transpose(1, 0) if graph_encoder_out is not None else None
            )
...
class LightConvDecoderLayer(nn.Module):
    ...
    def __init__(self, args, no_encoder_attn=False, kernel_size=0):
        ...
        if no_encoder_attn:
            ...
            if self.with_graph:
                self.g_attention = MultiheadAttention(self.embed_dim, args.graph_attention_heads,
                                                      dropout=args.attention_dropout, encoder_decoder_attention=True)
                self.g_attn_layer_norm = LayerNorm(self.embed_dim)
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state,
                g_encoder_out=None, g_encoder_padding_mask=None,
        ...
        if g_encoder_out is not None:
            g_hidden, g_encoder_out = g_encoder_out

        if self.with_graph:
          tmp = g_hidden.unsqueeze(1)
          tmp = tmp.repeat(1, x.shape[0],1).permute(1,0,2)
          x = x + tmp
        residual = x
        ...
        if self.encoder_attn is not None:
            ...
            if self.with_graph:
                x_graph, _ = self.g_attention(query=x, key=g_encoder_out, value=g_encoder_out,
                                              key_padding_mask=g_encoder_padding_mask,
...
def base_architecture(args):
    ...
    args.decoder_kernel_size_list = getattr(args, 'decoder_kernel_size_list', [3, 7, 15])
