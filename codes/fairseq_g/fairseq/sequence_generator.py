...
class SequenceGenerator(object):
    ...
    @torch.no_grad()
    def generate(self, models, sample, args, **kwargs):
        ...
        return self._generate(model, sample, args, **kwargs)
    ...
    def _generate(
        ...
        if args.with_amr:
            encoder_outs, g_encoder_out = model.forward_encoder(encoder_input, args=args)
        else:
            encoder_outs = model.forward_encoder(encoder_input, args=args)
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        # print(f"seq_gen.py:138, new_order.shape={new_order.shape}")
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = model.reorder_encoder_out(encoder_outs, new_order)
        if args.with_amr:
            g_encoder_out = model.reorder_encoder_out(g_encoder_out, new_order, is_graph=True)

        # print(f"encoder_output.shape={encoder_outs['encoder_out'].shape}")
        # print(f"encoder_output.shape={g_encoder_out['encoder_out'].shape}")
        # initialize buffers
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                if args.with_amr:
                    g_encoder_out = model.reorder_encoder_out(g_encoder_out, reorder_state, is_graph=True)
            if args.with_amr:
                lprobs, avg_attn_scores = model.forward_decoder(
                    tokens[:, :step + 1], encoder_outs, g_encoder_out[0], temperature=self.temperature,
                )
            else:
                lprobs, avg_attn_scores = model.forward_decoder(
                    tokens[:, :step + 1], encoder_outs, None, temperature=self.temperature,
                )
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_encoder(self, encoder_input, args):
        if not self.has_encoder():
            return None
        if args.with_amr:
            src_outputs = [model.encoder(encoder_input['src_tokens'], encoder_input['src_lengths']) for model in
                           self.models]
            graph_outputs = [model.graph_encoder(encoder_input['graph_tokens'], encoder_input['graph_lengths']) for
                             model in self.models]
            return src_outputs, graph_outputs
        return [model.encoder(encoder_input['src_tokens'], encoder_input['src_lengths']) for model in self.models]

    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, g_encoder_out=None, temperature=1.):
        if len(self.models) == 1:
            return self._decode_one(
                ...
                log_probs=True,
                graph_encoder_out=g_encoder_out,
    ...
    def _decode_one(
            ...
            temperature=1., graph_encoder_out=None):
        if self.incremental_states is not None:
            decoder_out = list(model.forward_decoder(
                tokens, encoder_out=encoder_out,graph_encoder_out=graph_encoder_out, incremental_state=self.incremental_states[model],
            ))
        else:
    ...
    def reorder_encoder_out(self, encoder_outs, new_order, is_graph=False):
        if not self.has_encoder():
            return
        return [
            model.encoder.reorder_encoder_out(encoder_out, new_order, is_graph)
            for model, encoder_out in zip(self.models, encoder_outs)
        ]
