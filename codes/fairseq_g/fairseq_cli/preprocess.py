...
def main(args):
    ...
    if args.joined_dictionary:
        ...
        if args.srcdict:
            ...
            # join node and edge vocab
            if args.amr:
                amr_dict = build_dictionary([train_path(args.amr)], src=True)
        if target:
            if args.tgtdict:
    ...
    if args.amr:
        amr_dict.save(dict_path(args.amr))
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Binarizer.binarize(
                ...
                offset=0, end=offsets[1], is_amr=is_amr
            )
        )
        if num_workers > 1:
            pool.join()
    ...
    def make_all_alignments():
        if args.trainpref and os.path.exists(args.trainpref + "." + args.align_suffix):
            make_binary_alignment_dataset(args.trainpref + "." + args.align_suffix, "train.align",
                                          num_workers=args.workers)
        if args.validpref and os.path.exists(args.validpref + "." + args.align_suffix):
            make_binary_alignment_dataset(args.validpref + "." + args.align_suffix, "valid.align",
                                          num_workers=args.workers)
        if args.testpref and os.path.exists(args.testpref + "." + args.align_suffix):
            make_binary_alignment_dataset(args.testpref + "." + args.align_suffix, "test.align",
                                          num_workers=args.workers)

    make_all(args.source_lang, src_dict)
    if args.amr:
        make_all(args.amr, amr_dict)
    if target:
